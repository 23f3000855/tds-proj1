...
def deploy_new_repo(repo_name, repo_path, files):
    ...
    run_command(['git', 'remote', 'add', 'origin', f'https://{GITHUB_USER}:{GITHUB_TOKEN}@github.com/{GITHUB_USER}/{repo_name}.git'], repo_path)
    run_command(['git', 'push', '-u', 'origin', 'main', '--force'], repo_path)
    
    
    run_command(['gh', 'pages', 'publish', '--branch', 'main', f'--repo={GITHUB_USER}/{repo_name}'], repo_path)
